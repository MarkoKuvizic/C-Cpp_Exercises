...
int GameOfLife::nextIterParallel(int _row, int _col, int _width) {
	...
	if (_width > grain_size) {
		task_group tg;
		tg.run([&] {rebornCells += nextIterParallel(_row + _width/ 2, _col + _width/ 2, _width / 2); });
		tg.run([&] {rebornCells += nextIterParallel(_row, _col + _width/ 2, _width / 2); });
		tg.run([&] {rebornCells += nextIterParallel(_row + _width/ 2, _col, _width / 2); });
		tg.run([&] {rebornCells += nextIterParallel(_row, _col , _width / 2); });
		tg.wait();
	}
	else {
